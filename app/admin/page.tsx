...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      if (supabase) {
        const [{ data: orderData, error: orderError }, { count, error: productError }] =
          await Promise.all([
            supabase
              .from("orders")
              .select(
                "order_number,total,status,payment_status,created_at,tracking_number,customer_name,customer_email,customer_phone,order_items(quantity)"
              )
  ...
  return (
    <main className="container-shop py-10 sm:py-14">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-3 text-neutral-500">
            {supabase ? "Live store overview from Supabase." : "Store overview and local demo order management."}
          </p>
        </div>
      ...
      <div className="mt-8 rounded-2xl border border-black/5 bg-white p-4 text-sm text-neutral-500 shadow-sm">
        ...
        {supabase
          ? "Orders and catalogue counts are loaded from Supabase. Status changes are protected by the admin API."
          : "Supabase is not configured, so this dashboard reads demo orders saved in the browser."}
      </div>
      ...
      <section className="mt-8 rounded-[32px] bg-white p-6 shadow-soft sm:p-8">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black">Recent orders</h2>
          </div>
          <select
            value={filter}
            onChange={(event) => setFilter(event.target.value)}
            className="rounded-full border border-neutral-200 bg-[#faf9f6] px-4 py-2.5 text-sm font-semibold outline-none"
          >
        ...
        {loading ? (
          <div className="py-14 text-center text-sm text-neutral-500">Loading dashboard…</div>
        ) : filtered.length === 0 ? (
          <div className="py-14 text-center text-sm text-neutral-500">No orders match this filter.</div>
        ) : (
          <div className="mt-6 overflow-x-auto">
            <table className="w-full min-w-[980px] text-left text-sm">
              <thead>
                <tr className="border-b border-black/5 text-xs uppercase tracking-wider text-neutral-400">
              ...
              <tbody>
                {filtered.map((order) => (
                  <OrderRow key={order.orderNumber} order={order} saving={saving === order.orderNumber} onSave={updateOrder} />
                ))}
              </tbody>
            </table>
          </div>
...
function OrderRow({
  ...
  order: Order;
  saving: boolean;
  onSave: (orderNumber: string, status: string, trackingNumber: string) => void;
}) {
    ...
    <tr className="border-b border-black/5 last:border-0">
      ...
      <td className="py-4">
        <select
          ...
          className="rounded-lg border border-neutral-200 bg-[#faf9f6] px-2.5 py-2 text-xs font-bold outline-none"
      ...
      <td className="py-4">
        <div className="flex gap-2">
          <input
            ...
            className="w-32 rounded-lg border border-neutral-200 bg-[#faf9f6] px-2.5 py-2 text-xs outline-none"
            disabled={!supabase || saving}
          />
          <button
            type="button"
            onClick={() => onSave(order.orderNumber, status, tracking)}
            disabled={!supabase || saving}
            className="inline-flex items-center gap-1 rounded-lg bg-black px-2.5 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"
          >
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-[24px] bg-white p-5 shadow-soft">
      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-neutral-100">{icon}</div>
      <p className="mt-5 text-xs font-bold uppercase tracking-wider text-neutral-400">{label}</p>
...
function QuickLink({ href, icon, title, text }: { href: string; icon: React.ReactNode; title: string; text: string }) {
  return (
    <Link href={href} className="group rounded-[28px] bg-white p-6 shadow-soft transition hover:-translate-y-1">
      <div className="flex items-center justify-between">
        <div className="flex h-11 w-11 items-center justify-center rounded-full bg-neutral-100">{icon}</div>
        <span className="text-neutral-300 transition group-hover:text-black">→</span>
      </div>
